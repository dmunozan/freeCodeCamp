...
function truthCheck(collection, pre) {
  // Variable initiated to true
  var allTrue = true;

  // Loop through collection while allTrue checking if the property exists and the value resolves to true
  for (var i = 0;allTrue && (i < collection.length); i++) {
    allTrue = (collection[i].hasOwnProperty(pre) && Boolean(collection[i][pre]));
  }
  return allTrue;
}
